...
\chapter{Introduction}
    ...
    With the rapid development of the Internet of Things (IoT) and wireless communication technologies, wireless sensor networks (WSNs) have been widely applied in environmental monitoring, smart agriculture, post-disaster detection, military reconnaissance, and other fields~\cite{Qin2019}, \cite{Zhang2023}, \cite{Xiang2023UAVMEC}. In these applications, ground nodes (GNs) are mainly responsible for collecting environmental parameters, such as temperature, humidity, air quality, and water quality. In such IoT applications, each GN usually generates lightweight data packets [14], [15], [16]. However, due to limited battery capacity and hardware cost, GNs are often unable to support long-distance wireless transmission. Therefore, transmitting sensing data back to a data center remains a major challenge [3].
    
    
    \begin{enumerate}
    	\item A trajectory planning method is developed to reduce the MCT of single-UAV-assisted WSN data collection by considering both the inter-region flight path and the intra-region data collection trajectory.
    	\item A global-direction-guided local trajectory adjustment method is introduced. The method enables the UAV to adjust the data collection trajectory within each GN while remaining close to the shortest geometric path, thereby reducing unnecessary flight distance.
    	\item A flexible service switching strategy is developed for consecutive GNs with overlapping communication ranges. Instead of restricting service switching to the boundary of an individual communication circle, the proposed strategy allows the UAV to utilize the shared coverage region between neighboring GNs as a switching area, thereby reducing redundant boundary transitions and unnecessary back-and-forth movement.
        \item Simulation results show that the proposed method reduces the overall MCT compared with the benchmark methods under different data requirements and GN distributions. The results further indicate that global geometric guidance and overlap-aware service switching contribute to reducing both flight distance and mission completion time.
    \end{enumerate}

    The remainder of this thesis is organized as follows. Chapter 2 reviews related studies on trajectory planning for UAV-assisted WSN data collection. Chapter 3 introduces the system model, including the communication model and the UAV flight model. Chapter 4 describes the proposed trajectory planning method. Chapter 5 presents the simulation setup, benchmark methods, and performance evaluation. Finally, Chapter 6 concludes this thesis and discusses possible future research directions.
%------------------------- Related Works -------------------------%
